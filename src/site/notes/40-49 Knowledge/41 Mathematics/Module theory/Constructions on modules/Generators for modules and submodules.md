---
{"dg-publish":true,"permalink":"/40-49-knowledge/41-mathematics/module-theory/constructions-on-modules/generators-for-modules-and-submodules/","tags":["module_theory"],"updated":"2025-10-09T14:31:47-07:00","dg-note-properties":{"last updated":"2025-10-09T14:31:47-07:00","related":["[[40-49 Knowledge/41 Mathematics/Module theory/Constructions on modules/Free modules]]","[[40-49 Knowledge/41 Mathematics/Module theory/Basic definitions and examples/Submodules]]","[[40-49 Knowledge/41 Mathematics/Module theory/Basic definitions and examples/Module morphisms]]"],"tags":["module_theory"]}}
---
...
If there exists a *finite* set $X$ that generates $M$, then we say $M$ is **finitely generated**.
{ #758e0b}


If there exists a singleton set that generates $M$, then we say $M$ is **cyclic**. In this case, this is equivalent to the existence of a single element $m_0\in M$ such that every element $m\in M$ can be expressed in the form $m=r m_0$ for some $r\in R$; as before, this expression does *not* have to be unique.

Connecting everything back to free modules once more, we see that $M$ is generated by $X$ exactly when the module morphism $F(X)\to M$ (corresponding to the inclusion $X\to U(M)$) is surjective. The module $M$ is cyclic exactly when there is a surjection $F(\{\bullet\})\to M$, where $\{\bullet\}$ is any singleton set.
