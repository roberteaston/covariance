---
last updated: 2023-10-05T14:29:54-07:00
related:
...
In terms of elements, this is just like a basis for a vector space. If $A$ is a subset of the elements of $M$, then $M$ is free on $A$ exactly when the elements of $A$ [[Generators for modules and submodules|generate]] $M$ and are $R$-linearly independent in $M$; the first condition guarantees that every $m\in M$ can be written as $\displaystyle m= \sum_{a\in A}r_a a$ for some $r_a\in A$, while the second condition guarantees that such an expression is unique.
