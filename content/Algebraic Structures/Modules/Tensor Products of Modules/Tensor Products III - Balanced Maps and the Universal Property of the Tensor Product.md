---
last updated: 2023-11-11T13:49:19-08:00
related:
...
Before we check the compatibilities with the ring actions, [[Bimodules#Other maps between bimodules|recall]] how the abelian group $\operatorname{Hom}_T(N,P)$ was given the structure of an $(R,S$)-bimodule: the left $R$-action arose from the left $R$-action on $P$, while the right $S$-action arose from the left $S$-action on $N$. In terms of elements, if $f:N\to P$ is morphism of right $T$-modules, then for each $r\in R$ we defined the map $r\cdot f$ by
...
We have not yet shown that $\tau_P$ is natural (in $P$), nor that it is bijective. We'll leave verifying naturality as [[The universal property of tensor products|an exercise]], but we will verify $\tau_P$ is a bijection by constructing the inverse map.
