...
Every ring $R$ is a left $R$-module, using the action of left multiplication on itself. In this context, the [[Submodules|submodules]] of $R$ (when considered as a left $R$-module) are exactly the left ideals of $R$ (when considered as a ring).
...
A given abelian group can even have different module structures for the same ring, $R$.  Being specific about the type of module (i.e., the ring doing the acting) can be important when talking about [[Module morphisms|module morphisms]].
...
Suppose $M$ is an $R$-module and $I\subseteq R$ is a two-sided ideal of $R$. Suppose $I$ [[Annihilators|annihilates]] $M$, i.e., $im=0$ for all $i\in I$ and $m\in M$. Then we can define an $R/I$-module structure on $M$ in the obvious way: $(r+I)m = rm$ for $r\in R$ and $m\in M$.

For example, if $M$ is a finite abelian group of order $n$, then $nm=0$ for every $m\in M$, i.e., the ideal $(n)\subseteq {\bf Z}$ [[Annihilators|annihilates]] $M$. We can therefore also view $M$ as a ${\bf Z}/n{\bf Z}$-module.
