---
last updated: 2023-11-01T11:42:06-07:00
related:
...
As the name implies, the $R$-module $\mathcal{T}(M)$ has a (natural) structure of an $R$-[[Algebras|algebra]]. The multiplication on simple tensors is defined by concatenation of tensors:
...
The multiplication on sums of tensors is defined via the distributive laws. Note that this multiplication satisfies $\mathcal{T}^i(M)\mathcal{T}^j(M)\subseteq \mathcal{T}^{i+j}(M)$. In other words, the tensor algebra $\mathcal{T}(M)$ has the structure of a [[Graded rings|graded ring]].
